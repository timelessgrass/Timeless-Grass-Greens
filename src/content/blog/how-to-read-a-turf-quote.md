---
status: published
title: How to Read a Turf Quote | TIMELESS Grass & Greens
description: Use a turf quote checklist to compare the area, product, preparation, drainage, edges, disposal and terms. Resolve open items before approving work.
h1: What should an artificial turf quote include?
topic: Costs and planning
published: '2026-09-10'
updated: '2026-09-12'
answer:
  question: What should an artificial turf quote include?
  answer: A useful turf quote identifies the measured area, layout, exact materials and work included in the price. It should explain preparation, drainage, seams, edges, removal and cleanup, along with timing and payment terms. Compare those details before comparing totals, and put agreed changes in writing.
faq:
  - q: Why does base depth matter on a quote?
    a: 'Because base preparation affects drainage and surface condition, and it is covered once the turf goes down. Rules that spell it out use numbers: <a href="https://www.thorntonwater.com/wp-content/uploads/2026/02/2026-residential_artificial_turf_info__rules-1.pdf" target="_blank" rel="nofollow noopener">Thornton''s rebate</a> requires at least 3 inches of compacted aggregate, and <a href="https://flrules.org/gateway/readFile.asp?sid=0&tid=30839130&type=1&file=62-308.100.doc" target="_blank" rel="nofollow noopener">Florida''s standard</a> requires washed subgrade material and permeable layers. A quote that only says "base prep" can''t be compared.'
  - q: Who should deal with the sprinkler lines?
    a: 'The quote should say. Utilities mark only the lines they own: <a href="https://www.colorado811.org/residential-digging" target="_blank" rel="nofollow noopener">Colorado 811</a> says private lines such as sprinklers and landscape lighting aren''t marked, and <a href="https://sc811.com/homeowners/faqs-homeowners/" target="_blank" rel="nofollow noopener">SC811</a> says the same of lines running past the meter. Someone has to find, cap or reroute them before the digging starts.'
  - q: How should payments be structured?
    a: The FTC advises against paying the full amount up front, and against making the final payment until the work is done and you're satisfied. Some states limit down payments, and your state or local consumer agency can tell you the rule where you live.
  - q: Does a turf installer need a license?
    a: It depends on the jurisdiction and full scope. In South Carolina, <a href="https://www.scstatehouse.gov/code/t40c011.php" target="_blank" rel="nofollow noopener">Section 40-11-30</a> requires a license for general contracting over $10,000. That does not establish the classification of every turf installation. Give LLR the address, property type, scope and contract value to confirm the applicable Board and category, including separate residential or specialty requirements. Do not infer an exemption from a lower quote total. Colorado has no state contractor or turf license, and <a href="https://www.flsenate.gov/Laws/Statutes/2025/489.105" target="_blank" rel="nofollow noopener">Florida's contractor categories</a> don't include turf. Irrigation or electrical work on the same job can need a license of its own.
sources:
  - label: 'Federal Trade Commission (July 2022): How To Avoid a Home Improvement Scam'
    url: https://consumer.ftc.gov/articles/how-avoid-home-improvement-scam
    checked: '2026-09-10'
  - label: 'Colorado 811: residential digging requirements'
    url: https://www.colorado811.org/residential-digging
    checked: '2026-09-10'
  - label: 'Sunshine 811 (Florida): when can I dig?'
    url: https://sunshine811.com/when-can-i-dig
    checked: '2026-09-10'
  - label: 'SC811: frequently asked questions for property holders'
    url: https://sc811.com/homeowners/faqs-homeowners/
    checked: '2026-09-10'
  - label: 'Thornton Water: 2026 Artificial Turf Rules for Water-Wise Landscape Rebate'
    url: https://www.thorntonwater.com/wp-content/uploads/2026/02/2026-residential_artificial_turf_info__rules-1.pdf
    checked: '2026-09-08'
  - label: 'Rule 62-308.100, Florida Administrative Code: adopted rule text'
    url: https://flrules.org/gateway/readFile.asp?sid=0&tid=30839130&type=1&file=62-308.100.doc
    checked: '2026-09-08'
  - label: '21 NCAC 28B .0503(b), North Carolina Landscape Contractors'' Licensing Board: artificial turf installation standard'
    url: http://reports.oah.state.nc.us/ncac/title%2021%20-%20occupational%20licensing%20boards%20and%20commissions/chapter%2028%20-%20landscape%20contractors/chapter%2028%20rules.pdf
    checked: '2026-09-08'
  - label: 'New Jersey Department of Environmental Protection (2025): synthetic turf and the current science'
    url: https://dep.nj.gov/wp-content/uploads/dsr/synthetic-turf-report-2025.pdf
    checked: '2026-09-10'
  - label: 'SC Code Title 40, Chapter 11: Contractors'' Licensing Board'
    url: https://www.scstatehouse.gov/code/t40c011.php
    checked: '2026-09-08'
  - label: 'Colorado DORA, Division of Professions and Occupations: regulated professions'
    url: https://dpo.colorado.gov/About
    checked: '2026-09-08'
  - label: 'Fla. Stat. 489.105: definition of contractor'
    url: https://www.flsenate.gov/Laws/Statutes/2025/489.105
    checked: '2026-09-08'
  - label: 'UC Master Gardeners of Alameda County (March 2026): artificial turf, what to consider'
    url: https://ucanr.edu/blog/over-fence-alameda-county/article/artificial-turf-what-consider
    checked: '2026-09-10'
related:
  services:
    - residential-turf
    - pet-turf
    - putting-greens
  markets:
    - denver-metro
    - grand-strand
    - northeast-florida
  guides:
    - turf-installer-licensing-by-state
    - florida-turf-infill-rules
publicReferences: []
---

## Start with how the price was made

A quote can only be as specific as the visit behind it. Ours starts with a free visit to measure the space and check the ground. Then you get a custom layout to approve, turf options side by side, and a written price.

Whoever you get quotes from, the same test applies: can you tell from the paper exactly what you're getting?

The FTC's advice for any home improvement job sets the floor. A written estimate should describe the work, the materials, the completion date and the price. The contract should add a start date, and put in writing any promises made about the scope of the work and the cost of labor and materials.

Turf adds lines of its own. Here's what each should say.

## The area and the layout

- **Square footage, measured on site.**
- **A drawing**: where the turf goes, where it stops, and what borders it, whether rock, pavers or concrete.
- **Everything that isn't turf**: beds, paths, a gravel strip along the house.

You should approve that drawing before any work starts.

## What comes out, and who calls 811

- **What's removed**: grass and roots, rock, old turf, and how deep the dig goes.
- **Haul-away**: whether it's included.
- **The utility locate.** All three of our states require a locate request before digging: at least [two full business days](https://www.colorado811.org/residential-digging) ahead in Colorado, [two full business days](https://sunshine811.com/when-can-i-dig) in Florida, and [three full working days](https://sc811.com/homeowners/faqs-homeowners/) in South Carolina.
- **Private lines.** Utilities mark only the lines they own, so sprinkler lines and landscape lighting aren't marked. The quote should say who finds, caps or reroutes them.

## The base

This line should explain the preparation for the property rather than leaving it at two vague words. It should give:

- the material, such as crushed rock
- the depth
- how it's compacted
- the slope, and which way the water runs

Where rules spell this out, they use specifics. [Thornton's rebate rules](https://www.thorntonwater.com/wp-content/uploads/2026/02/2026-residential_artificial_turf_info__rules-1.pdf) require at least 3 inches of compacted aggregate. [Florida's standard](https://flrules.org/gateway/readFile.asp?sid=0&tid=30839130&type=1&file=62-308.100.doc) requires washed subgrade material, permeable layers all the way down, and no compacting the soil beneath to the point that water can't soak in. North Carolina's [landscape licensing board](http://reports.oah.state.nc.us/ncac/title%2021%20-%20occupational%20licensing%20boards%20and%20commissions/chapter%2028%20-%20landscape%20contractors/chapter%2028%20rules.pdf) writes it into its rules: a sub-grade compacted and pitched to drain. Our [installer licensing guide](/guides/turf-installer-licensing-by-state/) makes the same point: ask for base depth and infill weight as numbers.

## Drainage

Turf drains through its backing, but the ground has the final say. As New Jersey's environment department puts it, a product that drains 10 inches an hour doesn't make the soil under it drain that fast. The quote should say where the water goes, whether into the soil, into a stone layer or out to a drain, and how the ground was checked. For a dog run, look for flow-through backing over a free-draining base.

## The turf and the infill

- **The exact product**: name, pile height, color and backing. If you saw options side by side, the quote should name the one you chose. Ours is premium American-made turf.
- **Backing that drains.** Thornton's rules, for one, require at least 2 inches an hour.
- **Infill type and amount.** In Florida, single-family lots may use only clean silica sand, rock, shell or other natural material, with rubber allowed only under playground equipment. Our [Florida infill guide](/guides/florida-turf-infill-rules/) covers the rule.

## Seams and edges

- **Seams**: how they're joined. Thornton's rules require glued, not sewn, seams.
- **Edges**: how they're held down. North Carolina's standard calls for a perimeter attachment system, and Florida's requires anchoring at all edges and seams.
- **Direction**: North Carolina's standard also has the turf rolled out with the nap facing one way.
- **Borders**: what the turf meets at its edges, and who supplies it.

## Cleanup and care

- **Cleanup**: leftover base, offcuts and packaging hauled away, and the site left clean.
- **Care**: a walkthrough of what the lawn needs. The [UC Master Gardeners](https://ucanr.edu/blog/over-fence-alameda-county/article/artificial-turf-what-consider) list the basics: cleaning, rinsing and brushing weekly or monthly, clearing leaves before they're ground in, and topping up the infill over time.

## Dates, payments and surprises

- **Dates**: a start date and a completion date.
- **Payments**: the FTC advises against paying in full up front, and against the final payment until the work is done and you're satisfied.
- **Surprises**: ask what happens if the crew finds rock, roots or buried debris under the old lawn, and put the answer in writing.

## Comparing two quotes

Put them side by side, line by line. If one leaves out the base depth or the infill amount, it isn't comparable, whatever the total says. Ask for the missing numbers before you decide.

If you're weighing doing it yourself instead, our piece on [DIY against hiring an installer](/blog/diy-vs-hiring-a-turf-installer/) walks through the job.

## The checklist

1. Area measured on site, with a drawing you approve
2. What comes out, how deep, and haul-away
3. The 811 locate, and who handles sprinkler and lighting lines
4. Base: material, depth, compaction and slope
5. Drainage: where the water goes
6. Turf: product, pile height and backing
7. Infill: type and amount
8. Seams, edges and borders
9. Cleanup
10. A care walkthrough
11. Dates and a payment schedule

Book the free visit for turf options, a custom layout and a written price. Use this checklist to ask what is included and resolve any open items before you approve the job.
