...
- **The form is a wizard.** `EstimateWizard` is the one estimate form (`quote`), rendered in a
  ...
  posting to `/api/lead`, which sends it on to `/thanks/`. No Netlify Forms (Ty). Motion: the sheet slides up on phones, steps slide, the done seal draws.
...
## Reviews, before/after and the lead pipeline, September 14, 2026

- **Reviews** (`src/data/reviews.ts`, `Reviews.astro`, `ReviewCard.astro`): customers' words as written, first name and last initial, five stars on each because each quoted review gave five. TIMELESS reviews are from the Denver Google profile (screenshots from Ty). NoCo Turf's are the sister company's and every card says so (Brian offered them on the call at 00:40:38; Ty asked for the citation). Still no average, no count and no Review schema. The "Read our reviews on Google" link shows only beside TIMELESS reviews.
- **Where they show:** the homepage after the before/after, the Denver metro hub only (the reviews are on the Denver profile), service pages with two or more matching reviews, and two short ones under the form on /estimate/.
- **Before/after** (`BeforeAfter.astro`) is a drag slider, replacing the scroll wipe. A mouse drags at once; a finger drags sideways or taps, and up-and-down still scrolls (`touch-action: pan-y`). A visually hidden range input gives keyboard and screen-reader control. It sweeps once on first view (expo.out) unless reduced motion is set. Pair ba-1 is a customer's own photos from their Google review, cropped and scaled so the frames line up.
- **Leads** (`netlify/functions/lead.mjs` at `POST /api/lead`, `netlify/functions/lib/lead-routing.mjs`): the form posts straight to the site's own endpoint, with no Netlify Forms (Ty's rule). It works out the area (ZIP, then town, state, then the page's market), the lane (home, bid, check-area), the priority and the call-prep questions, renders Brian's email, and posts the lead to Make scenario 6272892. Make adds every real lead to the TTM portal (client Timeless, campaign "Website — Turf Installation Lead") and emails timelessgrass@gmail.com from the route for its lane; test leads go to the agency inbox only. If Make does not take a lead, the wizard keeps the answers and offers the phone number, and a no-JavaScript visitor gets a page with the number. Tests: `tests/lead-routing.test.mjs`. After adding towns, run `node scripts/build-lead-places.mjs`.
- The form also carries `market` and the visit's campaign tags (UTM, gclid, gbraid, wbraid, fbclid, first referrer), kept in session storage for the tab. The privacy page says so.
