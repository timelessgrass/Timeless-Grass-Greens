...
- **Leads** (`netlify/functions/submission-created.mjs`, `netlify/functions/lib/lead-routing.mjs`): Netlify runs the function on each verified "quote" submission. It works out the area (ZIP, then town, state, then the page's market), the lane (home, bid, check-area), the priority and the call-prep questions, renders Brian's email, and posts the lead to Make scenario 6272892. Make adds every real lead to the TTM portal (client Timeless, campaign "Website — Turf Installation Lead") and emails timelessgrass@gmail.com from the route for its lane; test leads go to the agency inbox only. Tests: `tests/lead-routing.test.mjs`. After adding towns, run `node scripts/build-lead-places.mjs`.
